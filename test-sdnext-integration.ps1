# Script de test pour vérifier l'intégration SDNext
Write-Host "🎨 Test de l'intégration SDNext" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "🎯 Configuration SDNext implémentée:" -ForegroundColor Yellow
Write-Host "   ✅ API check-auth modifiée pour gérer SDNext" -ForegroundColor White
Write-Host "   ✅ Redirection vers sdnext.iahome.fr avec token" -ForegroundColor White
Write-Host "   ✅ Accès interdit hors iahome.fr" -ForegroundColor White
Write-Host "   ✅ AuthorizedAccessButton mis à jour pour SDNext" -ForegroundColor White
...
Write-Host "   ✅ Configuration Docker ajoutée (ghcr.io/automatic1111/stable-diffusion-webui:latest)" -ForegroundColor White
...
Write-Host "🔐 Processus d'accès SDNext:" -ForegroundColor Cyan
Write-Host "1. Utilisateur clique sur le bouton d'accès SDNext" -ForegroundColor White
...
Write-Host "4. Ouverture de sdnext.iahome.fr?token=XXX dans un nouvel onglet" -ForegroundColor White
Write-Host "5. API check-auth valide le token et redirige vers SDNext" -ForegroundColor White
Write-Host "6. Accès direct bloqué si pas de token ou origine incorrecte" -ForegroundColor White
Write-Host ""

Write-Host "⚠️  Note sur le service SDNext:" -ForegroundColor Yellow
Write-Host "   Le service SDNext nécessite des ressources importantes (GPU/CPU)" -ForegroundColor White
Write-Host "   Il n'est pas démarré automatiquement pour éviter la surcharge" -ForegroundColor White
Write-Host "   Pour le démarrer manuellement: docker-compose -f docker-services/docker-compose.services.yml up -d sdnext" -ForegroundColor White
Write-Host ""
...
Write-Host "4. Cliquez sur le bouton d'accès SDNext" -ForegroundColor White
Write-Host "5. Vérifiez l'ouverture dans un nouvel onglet avec token" -ForegroundColor White
Write-Host "6. Testez l'accès direct à sdnext.iahome.fr (doit rediriger vers login)" -ForegroundColor White
Write-Host ""

Write-Host "✅ Intégration SDNext complète !" -ForegroundColor Green
Write-Host "🎨 SDNext fonctionne exactement comme LibreSpeed, MeTube, PsiTransfer, PDF, StableDiffusion, RuinedFooocus et ComfyUI avec token temporaire" -ForegroundColor Green
