...
function showChoicePanel(eventData) {
  ...
  document.getElementById("choice-event-meta").textContent =
    "Application proposée par Régis Pailler, pour IAHome.fr";
  ...
  var videoStudioUrl = new URL("./video-studio.html", window.location.href);
  videoStudioUrl.searchParams.set("eventId", eventData.id);
  if (token) videoStudioUrl.searchParams.set("token", token);
  ...
  var btnVideo = document.getElementById("btn-short-video");
  var btnGallery = document.getElementById("btn-gallery");
  if (btnPhoto) btnPhoto.href = studioUrl.toString();
  if (btnVideo) btnVideo.href = videoStudioUrl.toString();
