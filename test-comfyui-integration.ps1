# Script de test pour vérifier l'intégration ComfyUI
Write-Host "🎨 Test de l'intégration ComfyUI" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "🎯 Configuration ComfyUI implémentée:" -ForegroundColor Yellow
Write-Host "   ✅ API check-auth modifiée pour gérer ComfyUI" -ForegroundColor White
Write-Host "   ✅ Redirection vers comfyui.iahome.fr avec token" -ForegroundColor White
Write-Host "   ✅ Accès interdit hors iahome.fr" -ForegroundColor White
Write-Host "   ✅ AuthorizedAccessButton mis à jour pour ComfyUI" -ForegroundColor White
...
Write-Host "   ✅ Configuration Docker ajoutée (yanwk/comfyui:latest)" -ForegroundColor White
...
Write-Host "🔐 Processus d'accès ComfyUI:" -ForegroundColor Cyan
Write-Host "1. Utilisateur clique sur le bouton d'accès ComfyUI" -ForegroundColor White
...
Write-Host "4. Ouverture de comfyui.iahome.fr?token=XXX dans un nouvel onglet" -ForegroundColor White
Write-Host "5. API check-auth valide le token et redirige vers ComfyUI" -ForegroundColor White
Write-Host "6. Accès direct bloqué si pas de token ou origine incorrecte" -ForegroundColor White
Write-Host ""

Write-Host "⚠️  Note sur le service ComfyUI:" -ForegroundColor Yellow
Write-Host "   Le service ComfyUI nécessite des ressources importantes (GPU/CPU)" -ForegroundColor White
Write-Host "   Il n'est pas démarré automatiquement pour éviter la surcharge" -ForegroundColor White
Write-Host "   Pour le démarrer manuellement: docker-compose -f docker-services/docker-compose.services.yml up -d comfyui" -ForegroundColor White
Write-Host ""
...
Write-Host "4. Cliquez sur le bouton d'accès ComfyUI" -ForegroundColor White
Write-Host "5. Vérifiez l'ouverture dans un nouvel onglet avec token" -ForegroundColor White
Write-Host "6. Testez l'accès direct à comfyui.iahome.fr (doit rediriger vers login)" -ForegroundColor White
Write-Host ""

Write-Host "✅ Intégration ComfyUI complète !" -ForegroundColor Green
Write-Host "🎨 ComfyUI fonctionne exactement comme LibreSpeed, MeTube, PsiTransfer, PDF, StableDiffusion et RuinedFooocus avec token temporaire" -ForegroundColor Green
