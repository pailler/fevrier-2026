...
if ($cloudflareProcess) {
    ...
    if ($startCloudflare -eq "O" -or $startCloudflare -eq "o") {
        ...
        Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PWD'; cloudflared tunnel --config cloudflare-active-config.yml run" -WindowStyle Minimized
        Start-Sleep -Seconds 5
...
Write-Host "   Get-Process -Name node | Where-Object { `$_.Path -like '*node*' } | Stop-Process -Force" -ForegroundColor Gray
Write-Host ""
