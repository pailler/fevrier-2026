# Script de test final pour PDF
Write-Host "📄 Test final de l'intégration PDF" -ForegroundColor Green
Write-Host ""
...
Write-Host "   ✅ Service PDF: https://pdf.iahome.fr" -ForegroundColor Green
...
Write-Host "1. Clic sur bouton PDF → Vérification quotas" -ForegroundColor White
Write-Host "2. Génération token JWT → Ouverture nouvel onglet" -ForegroundColor White
Write-Host "3. URL: pdf.iahome.fr?token=XXX" -ForegroundColor White
Write-Host "4. API check-auth valide token → Redirection PDF" -ForegroundColor White
Write-Host "5. Accès direct bloqué → Redirection login" -ForegroundColor White
Write-Host ""

Write-Host "✅ PDF intégré avec succès !" -ForegroundColor Green
Write-Host "📄 Même processus que LibreSpeed, MeTube et PsiTransfer : token + nouvel onglet + sécurité" -ForegroundColor Green