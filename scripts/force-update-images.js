// Script Node.js pour forcer la mise à jour de tous les modules avec leurs images JPG
// Ce script élimine définitivement les zones noires
...
// Mapping complet des modules vers leurs images JPG appropriées
const imageMapping = {
...
async function forceUpdateImages() {
  console.log('🔄 Force mise à jour de tous les modules avec leurs images JPG...');
  ...
  try {
    ...
    // Mettre à jour chaque module avec son image JPG appropriée
    for (const module of modules) {
      ...
      if (error) {
        ...
        console.log(`✅ ${module.title} mis à jour: ${module.image_url} → ${newImageUrl}`);
      }
    }
    ...
    if (finalError) {
      ...
      const validCount = finalModules.filter(m => 
        m.image_url && !m.image_url.includes('.svg') && !m.image_url.includes('iahome.fr')
      ).length;
      
      console.log(`\n🎉 ${validCount}/${finalModules.length} modules ont des images JPG valides`);
      
      if (validCount === finalModules.length) {
        console.log('🎊 Toutes les zones noires ont été éliminées !');
      } else {
        console.log('⚠️  Certains modules ont encore des problèmes d\'images');
      }
    }
