# Script pour installer Cloudflare Tunnel comme service Windows avec démarrage automatique
# Ce script doit être exécuté en tant qu'administrateur
# Emplacement : scripts/install-cloudflare-auto-start.ps1
...
if (-not $isAdmin) {
    ...
    Write-Host "`nRelance avec élévation des droits..." -ForegroundColor Yellow
...
Write-Host "║  avec démarrage automatique au boot                 ║" -ForegroundColor Cyan
...
if ($service) {
    ...
    Write-Host "   Réinstallation du service..." -ForegroundColor Yellow
...
try {
    ...
    if ($service) {
        ...
        Write-Host "   Démarrage : Automatique" -ForegroundColor Green
        Write-Host "   Statut : $($service.Status)" -ForegroundColor $(if ($service.Status -eq 'Running') {'Green'} else {'Yellow'})
        
        Write-Host "`n💡 Le service démarrera automatiquement au prochain démarrage de Windows" -ForegroundColor Green
        
        Write-Host "`n💡 Commandes utiles :" -ForegroundColor Cyan
        Write-Host "   - Vérifier le statut : .\scripts\install-cloudflare-auto-start.ps1 -Status" -ForegroundColor Gray
