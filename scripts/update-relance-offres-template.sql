-- Script pour mettre à jour le template "Relance offres IAHome" dans notification_settings
-- À exécuter dans Supabase : SQL Editor > New query > Coller et Run
-- Met à jour uniquement email_template_subject et email_template_body (colonnes utilisées pour l'envoi)
        ...
        <table role="presentation" style="max-width: 600px; width: 100%; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td class="mobile-padding" style="padding: 28px 24px;">
              ...
              <p style="color: #374151; font-size: 15px; font-style: italic; margin: 16px 0 0 0;">C'est votre studio de création numérique, prêt à être utilisé à tout moment.</p>
            </td>
          </tr>
          <tr>
            <td style="padding: 28px 24px; background-color: #047857;">
...
WHERE event_type = 'relance_offres_iahome';

-- Vérifier le résultat (optionnel)
-- Si la ligne n'existe pas, l'UPDATE ne modifie rien. Dans ce cas, exécutez d'abord l'INSERT ci-dessous.
-- Pour insérer si la ligne n'existe pas, décommentez et exécutez après l'UPDATE :

/*
INSERT INTO notification_settings (event_type, name, description, is_enabled, email_template_subject, email_template_body)
SELECT 'relance_offres_iahome', 'Relance offres IAHome', 'Email pour inciter les inscrits à découvrir les packs de tokens et souscrire aux offres', true, 'Vous utilisez déjà iahome — découvrez nos offres pour aller plus loin', $$...même HTML que ci-dessus...$$
WHERE NOT EXISTS (SELECT 1 FROM notification_settings WHERE event_type = 'relance_offres_iahome');
*/
