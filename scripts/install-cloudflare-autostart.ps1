# Script pour installer le demarrage automatique de Cloudflare Tunnel
...
$scriptPath = Join-Path $RootPath "scripts\start-cloudflare-auto-start.ps1"
$taskName = "IAHome-CloudflareTunnel"
...
try {
    ...
    # Description
    $description = "Demarre automatiquement Cloudflare Tunnel au demarrage de Windows (methode directe sans service)"
...
if ($task) {
    ...
    Write-Host "[INFO] Cloudflare Tunnel demarrera automatiquement au prochain demarrage de Windows" -ForegroundColor Cyan
    Write-Host "   La tache planifiee utilise la methode directe (sans service Windows)" -ForegroundColor Gray
    Write-Host "   qui a ete testee et fonctionne correctement" -ForegroundColor Gray
    
    Write-Host "`n[TEST] Pour tester maintenant:" -ForegroundColor Yellow
    Write-Host "   .\scripts\start-cloudflare-auto-start.ps1" -ForegroundColor Gray
