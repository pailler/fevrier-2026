# Script PowerShell pour vérifier et corriger les images des formations
# Exécuté le: $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")

Write-Host "🔄 Vérification et correction des images des formations..." -ForegroundColor Yellow

# Lire le contenu du script SQL
$sqlScript = Get-Content "scripts/check-formation-images.sql" -Raw
...
Write-Host "   • Vérifier toutes les formations existantes" -ForegroundColor White
Write-Host "   • Ajouter une image par défaut aux formations sans image" -ForegroundColor White
Write-Host "   • Créer l'article 'assistant-ia' s'il n'existe pas" -ForegroundColor White
