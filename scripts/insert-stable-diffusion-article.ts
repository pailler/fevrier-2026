/**
 * Script pour insérer l'article sur Stable Diffusion comme alternative aux chatbots commerciaux
 * Exécuter avec: npx tsx scripts/insert-stable-diffusion-article.ts
 */

import { createClient } from '@supabase/supabase-js';

// Valeurs par défaut hardcodées (mêmes que dans supabaseConfig.ts)
...
<div class="article-content">
  ...
  <p><strong>Prêt à découvrir Stable Diffusion ? <a href="/card/stablediffusion" style="color: #2563eb; text-decoration: underline;">Explorez notre module Stable Diffusion</a> et commencez à générer vos propres images dès aujourd'hui.</strong></p>
</div>
`;

async function insertArticle() {
  try {
    console.log('📝 Insertion de l\'article sur Stable Diffusion...');
    ...
    console.log('⏱️  Temps de lecture:', readTime, 'minutes');
    console.log('📊 Nombre de mots:', wordCount);
    console.log('\n🌐 URL de l\'article: https://iahome.fr/blog/' + data.slug);
  } catch (error: any) {
