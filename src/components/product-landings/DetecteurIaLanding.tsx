...
import { DETECTEUR_IA_CARD_URL } from '@/utils/productLandingHosts';
...
const concepts = [
  { icon: '📝', label: 'Texte', detail: 'Collez un paragraphe et obtenez un score IA / humain' },
  { icon: '📄', label: 'Fichiers', detail: 'Importez un document pour une analyse rapide' },
  { icon: '🖼️', label: 'Images', detail: 'Estimez si une image peut provenir d’une génération IA' },
  { icon: '📊', label: 'Scores clairs', detail: 'Probabilité, confiance et points d’attention' },
  { icon: '🔐', label: 'Accès sécurisé', detail: 'Ouverture via crédits IAHome et jeton d’accès' },
  { icon: '🇫🇷', label: 'En français', detail: 'Interface et retours pensés pour un usage quotidien' },
];

const useCases = [
  {
    title: 'Éducation & pédagogie',
    text: 'Sensibilisez aux contenus générés par IA sans outil complexe.',
  },
  {
    title: 'Éditeurs & créateurs',
    text: 'Vérifiez rapidement un texte reçu ou une image suspecte.',
  },
  {
    title: 'Usage personnel',
    text: 'Une estimation claire pour mieux lire ce que vous croisez en ligne.',
  },
];
    ...
    <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <nav className="flex flex-wrap items-center gap-1 sm:gap-2" aria-label="Navigation de la page">
          ...
          <a href={DETECTEUR_IA_CARD_URL} className="ml-1 rounded-lg bg-sky-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-400" {...externalTab}>
            Commencer
          </a>
        </nav>
      </div>
  ...
  return (
    <footer className="border-t border-white/10 bg-slate-950 py-10 text-sm">
      <div className="mx-auto flex max-w-5xl flex-col items-center gap-6 px-4 text-center sm:px-6 lg:px-8">
        ...
        <p className="text-slate-500">© {new Date().getFullYear()} IAHome — Détecteur de contenu IA</p>
      </div>
    </footer>
  );
}

export default function DetecteurIaLanding() {
  const { headline, description, faqs } = detecteurIaLandingSeo;
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-sky-950 text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <a href={DETECTEUR_IA_CARD_URL} className="rounded-xl bg-sky-500 px-6 py-3 text-base font-semibold text-white shadow-lg shadow-sky-900/40 transition hover:bg-sky-400" {...externalTab}>
              Accéder via IAHome
            </a>
      ...
      <section id="fonctionnalites" className="border-t border-white/10 bg-slate-950/40 py-16">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold sm:text-3xl">Une lecture claire du risque IA</h2>
          <p className="mt-3 max-w-2xl text-slate-300/80">Texte, fichier ou image : une estimation utile pour vérifier ou enseigner.</p>
      ...
      <section id="faq" className="border-t border-white/10 bg-slate-950/50 py-16">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10">
            <a href={DETECTEUR_IA_CARD_URL} className="inline-flex rounded-xl bg-sky-500 px-6 py-3 font-semibold text-white transition hover:bg-sky-400" {...externalTab}>
