...
export async function GET(request: NextRequest) {
  try {
    console.log('🔒 StableDiffusion Redirect: Vérification de sécurité');
    ...
    if (token) {
      ...
      // Vérifier que le module stablediffusion est visible dans /encours pour cet utilisateur
      const securityService = ModuleSecurityService.getInstance();
      const canAccess = await securityService.canAccessExternalApp(tokenData.created_by, 'stablediffusion');
      
      if (!canAccess) {
        const reason = await securityService.getAccessDenialReason(tokenData.created_by, 'stablediffusion');
      ...
      console.log('🔗 Redirection vers StableDiffusion');
    ...
    // Vérifier que le module stablediffusion est visible dans /encours pour cet utilisateur
    const securityService = ModuleSecurityService.getInstance();
    const canAccess = await securityService.canAccessExternalApp(session.user.id, 'stablediffusion');
    
    if (!canAccess) {
      const reason = await securityService.getAccessDenialReason(session.user.id, 'stablediffusion');
    ...
    console.error('❌ Erreur StableDiffusion Redirect:', error);
