...
export async function GET(request: NextRequest) {
  try {
    ...
    if (token) {
      ...
      const canAccess = await securityService.canAccessExternalApp(tokenData.created_by, 'comfyui');
      
      if (!canAccess) {
        const reason = await securityService.getAccessDenialReason(tokenData.created_by, 'comfyui');
    ...
    const securityService = ModuleSecurityService.getInstance();
    const canAccess = await securityService.canAccessExternalApp(session.user.id, 'comfyui');
    
    if (!canAccess) {
      const reason = await securityService.getAccessDenialReason(session.user.id, 'comfyui');
    ...
    console.error('❌ Erreur ComfyUI Redirect:', error);
