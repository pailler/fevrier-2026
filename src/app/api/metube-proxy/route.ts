...
export async function GET(request: NextRequest) {
  try {
    ...
    const targetUrl = `${METUBE_SERVICE_URL}${searchParams ? `?${searchParams}` : ''}`;
    ...
    console.error('Erreur proxy MeTube racine:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Erreur proxy MeTube POST racine:', error);
    return NextResponse.json(
      { error: 'Erreur interne du serveur' },
      { status: 500 }
    );
