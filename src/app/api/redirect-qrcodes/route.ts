...
export async function GET(request: NextRequest) {
  try {
    console.log('🔒 QR Codes Redirect: Vérification de sécurité');
    ...
    if (token) {
      ...
      // Vérifier que le module qrcodes est visible dans /encours pour cet utilisateur
      const securityService = ModuleSecurityService.getInstance();
      const canAccess = await securityService.canAccessExternalApp(tokenData.created_by, 'qrcodes');
      
      if (!canAccess) {
        const reason = await securityService.getAccessDenialReason(tokenData.created_by, 'qrcodes');
      ...
      // Rediriger vers QR Codes
      console.log('🔗 Redirection vers QR Codes');
      return NextResponse.redirect('https://qrcodes.iahome.fr', 302);
    }
    ...
    // Vérifier que le module qrcodes est visible dans /encours pour cet utilisateur
    const securityService = ModuleSecurityService.getInstance();
    const canAccess = await securityService.canAccessExternalApp(session.user.id, 'qrcodes');
    
    if (!canAccess) {
      const reason = await securityService.getAccessDenialReason(session.user.id, 'qrcodes');
    ...
    console.error('❌ Erreur QR Codes Redirect:', error);
