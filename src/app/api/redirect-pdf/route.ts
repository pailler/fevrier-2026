...
export async function GET(request: NextRequest) {
  try {
    console.log('🔒 PDF Redirect: Vérification de sécurité');
    ...
    if (token) {
      ...
      // Vérifier que le module pdf est visible dans /encours pour cet utilisateur
      const securityService = ModuleSecurityService.getInstance();
      const canAccess = await securityService.canAccessExternalApp(tokenData.created_by, 'pdf');
      
      if (!canAccess) {
        const reason = await securityService.getAccessDenialReason(tokenData.created_by, 'pdf');
      ...
      // Rediriger vers PDF
      console.log('🔗 Redirection vers PDF');
    ...
    // Vérifier que le module pdf est visible dans /encours pour cet utilisateur
    const securityService = ModuleSecurityService.getInstance();
    const canAccess = await securityService.canAccessExternalApp(session.user.id, 'pdf');
    
    if (!canAccess) {
      const reason = await securityService.getAccessDenialReason(session.user.id, 'pdf');
    ...
    console.error('❌ Erreur PDF Redirect:', error);
