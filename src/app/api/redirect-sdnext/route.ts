...
export async function GET(request: NextRequest) {
  try {
    ...
    if (token) {
      ...
      const canAccess = await securityService.canAccessExternalApp(tokenData.created_by, 'sdnext');
      
      if (!canAccess) {
        const reason = await securityService.getAccessDenialReason(tokenData.created_by, 'sdnext');
    ...
    const securityService = ModuleSecurityService.getInstance();
    const canAccess = await securityService.canAccessExternalApp(session.user.id, 'sdnext');
    
    if (!canAccess) {
      const reason = await securityService.getAccessDenialReason(session.user.id, 'sdnext');
    ...
    console.error('❌ Erreur SDNext Redirect:', error);
