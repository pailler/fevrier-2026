...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 Activation du module Générateur de Prompts IA pour l\'utilisateur:', targetUserId);

    // 1. Vérifier/Créer le module Générateur de Prompts IA
    ...
    if (moduleError || !existingModule) {
      ...
      console.log('✅ Module Générateur de Prompts IA créé:', moduleId);
    } else {
      moduleId = existingModule.id;
      console.log('✅ Module Générateur de Prompts IA trouvé:', moduleId);
    }
    ...
    if (existingAccess) {
      return NextResponse.json({
        success: true,
        message: 'Accès Générateur de Prompts IA déjà activé',
    ...
    const { data: accessData, error: createAccessError } = await supabase
      ...
      .insert([{
        ...
        module_id: moduleId,
        module_title: 'Générateur de Prompts IA',
        is_active: true,
    ...
    console.log('✅ Accès Générateur de Prompts IA créé avec succès:', accessData.id);

    return NextResponse.json({
      success: true,
      message: 'Générateur de Prompts IA activé avec succès',
    ...
    console.error('❌ Erreur activation Générateur de Prompts IA:', error);
    return NextResponse.json({ 
      success: false, 
