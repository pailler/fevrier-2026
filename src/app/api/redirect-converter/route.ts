...
export async function GET(request: NextRequest) {
  try {
    console.log('🔒 Converter Redirect: Vérification de sécurité');
    ...
    if (token) {
      ...
      // Vérifier que le module converter est visible dans /encours pour cet utilisateur
      const securityService = ModuleSecurityService.getInstance();
      const canAccess = await securityService.canAccessExternalApp(tokenData.created_by, 'converter');
      
      if (!canAccess) {
        const reason = await securityService.getAccessDenialReason(tokenData.created_by, 'converter');
      ...
      // Rediriger vers Converter
      console.log('🔗 Redirection vers Converter');
    ...
    // Vérifier que le module converter est visible dans /encours pour cet utilisateur
    const securityService = ModuleSecurityService.getInstance();
    const canAccess = await securityService.canAccessExternalApp(session.user.id, 'converter');
    
    if (!canAccess) {
      const reason = await securityService.getAccessDenialReason(session.user.id, 'converter');
    ...
    console.error('❌ Erreur Converter Redirect:', error);
