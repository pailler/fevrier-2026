...
export async function GET(request: NextRequest) {
  try {
    ...
    if (token) {
      ...
      const canAccess = await securityService.canAccessExternalApp(tokenData.created_by, 'cogstudio');
      
      if (!canAccess) {
        const reason = await securityService.getAccessDenialReason(tokenData.created_by, 'cogstudio');
      ...
      return NextResponse.redirect('https://cogstudio.iahome.fr', 302);
    }
    ...
    const securityService = ModuleSecurityService.getInstance();
    const canAccess = await securityService.canAccessExternalApp(session.user.id, 'cogstudio');
    
    if (!canAccess) {
      const reason = await securityService.getAccessDenialReason(session.user.id, 'cogstudio');
    ...
    console.error('❌ Erreur CogStudio Redirect:', error);
