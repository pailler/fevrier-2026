...
export async function POST(request: NextRequest) {
  try {
    console.log('🔔 Webhook Stripe reçu sur /api/webhooks/stripe');
...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  ...
  if (paymentType === 'token_purchase' || packageType === 'pack_standard') {
    ...
    console.log('🔄 Ajout de tokens via webhook (achat unique):', {
      userId: userData.id,
      tokens,
    ...
    // Ajouter les tokens à l'utilisateur (addition pour les achats uniques)
