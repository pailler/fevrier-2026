/** Préfixe public : Gradio servi sous iahome.fr/stablediffusion/embed → :7880. */
export const STABLEDIFFUSION_EMBED_PREFIX = '/stablediffusion/embed';

export const STABLEDIFFUSION_INTERNAL_URL =
  process.env.STABLEDIFFUSION_INTERNAL_URL || 'http://host.docker.internal:7880';

export function buildStableDiffusionUpstreamUrl(
  pathSegments: string[],
  search: string
): string {
  const path = pathSegments.filter(Boolean).join('/');
  const base = STABLEDIFFUSION_INTERNAL_URL.replace(/\/$/, '');
...
function internalOriginPatterns(): string[] {
  const origins = new Set<string>([
    ...
    'https://host.docker.internal:7880',
    'http://127.0.0.1:7880',
    'http://localhost:7880',
    'http://192.168.1.39:7880',
  ]);
  try {
    origins.add(new URL(STABLEDIFFUSION_INTERNAL_URL).origin);
  } catch {
    /* ignore */
  }
  return [...origins];
...
export function rewriteStableDiffusionPayload(body: string): string {
  const root = STABLEDIFFUSION_EMBED_PREFIX;
  // Slash final OBLIGATOIRE : sinon new URL('gradio_api/…', root) → /stablediffusion/gradio_api (404 Not Found)
  ...
  out = out.replace(
    /"api_prefix"\s*:\s*"\/stablediffusion\/embed\/gradio_api"/g,
    `"api_prefix":"${root}/gradio_api"`
  );
  // Forcer root avec slash final (écrase toute variante)
  out = out.replace(
    /"root"\s*:\s*"https:\/\/(?:www\.)?iahome\.fr\/stablediffusion\/embed\/?"/g,
    `"root":"${publicBase}"`
  );
  out = out.replace(
 ...
 */
export function rewriteStableDiffusionHtml(html: string): string {
  const root = STABLEDIFFUSION_EMBED_PREFIX;
...
function rewriteJson(t){
  ...
  if(t.indexOf("gradio_api")===-1&&t.indexOf("host.docker")===-1&&t.indexOf(":7880")===-1&&t.indexOf(o+"/")===-1&&t.indexOf('"root"')===-1)return t;
  var out=t;
...
document.addEventListener("click",function(ev){
  ...
  let out = rewriteStableDiffusionPayload(html);
  ...
  out = out.replace(
    /"root"\s*:\s*"https:\/\/iahome\.fr\/stablediffusion\/embed"/g,
    `"root":"${publicOrigin}${root}/"`
  );

  out = out.replace(
    /(\s(?:src|href|action)=["'])\/(?!\/)(?!stablediffusion\/embed(?:\/|"|\?|#))/gi,
    `$1${root}/`
  );

  // Gradio 6 Vite : src="./assets/…" — sans slash final sur /embed, ça devient /stablediffusion/assets (hors proxy)
  out = out.replace(
