...
# 3. Configurer les en-tetes de securite
Write-Host "`n3. Configuration des en-tetes de securite..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "   En-tetes de securite configures" -ForegroundColor Green
} catch {
    if ($_.Exception.Message -like "*already exists*") {
        Write-Host "   En-tetes deja configures" -ForegroundColor Yellow
    } else {
        Write-Host "   Erreur en-tetes: $($_.Exception.Message)" -ForegroundColor Red
    }
}
