# Script pour ajouter RuinedFooocus au démarrage automatique de Windows
# Exécutez ce script en tant qu'administrateur si nécessaire

Write-Host "🚀 Ajout de RuinedFooocus au démarrage automatique" -ForegroundColor Cyan
...
# Chemin vers le script de démarrage de RuinedFooocus
$scriptPath = Join-Path $PSScriptRoot "start-ruinedfooocus.ps1"
$scriptPath = Resolve-Path $scriptPath -ErrorAction SilentlyContinue

if (-not $scriptPath) {
    Write-Host "❌ Erreur: Impossible de trouver start-ruinedfooocus.ps1" -ForegroundColor Red
    Write-Host "   Chemin recherché: $PSScriptRoot\start-ruinedfooocus.ps1" -ForegroundColor Yellow
    exit 1
}
...
@echo off
cd /d "$PSScriptRoot"
powershell.exe -ExecutionPolicy Bypass -File "start-ruinedfooocus.ps1"
"@
...
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "✅ RuinedFooocus sera lancé automatiquement au prochain démarrage de Windows" -ForegroundColor Green
Write-Host ""
...
Write-Host "   pour que RuinedFooocus puisse démarrer correctement." -ForegroundColor Yellow
Write-Host ""
